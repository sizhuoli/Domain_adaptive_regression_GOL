...
from utils.diffusion import kReciprocalReRanking
...
def find_kNN(queries, samples, k=1, metric='cosine', reciprocal=False, rec_k1=20, rec_k2=6, rec_l = 0.1):
    """
    :param queries: BxNxC
    :param samples: BxMxC
    :param metric:
    ...
    if metric == 'L2':
        ...
        # if reciprocal:
        #     q_q_dist = torch.cdist(queries, queries)
        #     g_g_dist = torch.cdist(samples, samples)
        #     q_g_dist = torch.cdist(queries, samples)

    elif metric == 'cosine':
        queries = torch.nn.functional.normalize(queries, dim=-1)
        samples = torch.nn.functional.normalize(samples, dim=-1)

        dist_mat = 1 - torch.matmul(queries, samples.transpose(1,0))

        # if reciprocal:
        #     q_q_dist = torch.matmul(queries, queries.transpose(1,0))
        #     g_g_dist = torch.matmul(samples, samples.transpose(1,0))
        #     q_g_dist = torch.matmul(queries, samples.transpose(1,0))
    ...
    if not reciprocal:
        ...
        rerank = kReciprocalReRanking(queries, samples, cosine=metric=='cosine')
        final_dist, ori_dist = rerank.forward(k1 = rec_k1, k2 = rec_k2, l = rec_l) # check how different is the new knn from the old knn!!!
        vals, inds = torch.topk(-final_dist, k, dim=-1)

        # ipdb.set_trace()
        # test without reciprocal
        # _, inds_no = torch.topk(-ori_dist, k, dim=-1)
        # print(f'kNN with reciprocal: {inds[:2]}')
        # print(f'kNN without reciprocal: {inds_no[:2]}')
        # # overlap of inds, ignoring order
        # np.intersect1d(inds.cpu().numpy(), inds_no.cpu().numpy(), assume_unique=True).shape[0] / (k*2
